...
class MovieManager(models.Manager):
    def all_with_prefetch_persons(self):
        qs = self.get_queryset()
        qs.select_related('director', )
        return qs.prefetch_related('writers', 'actors', )
...
class Movie(models.Model):
    ...
    writers = models.ManyToManyField('Person', related_name='writer', blank=True)
    actors = models.ManyToManyField('Person', through='Role', related_name='acting', blank=True)
    ...
    def get_absolute_url(self):
        return reverse('core:MovieDetail', kwargs={'pk': self.pk})


class VoteManager(models.Manager):
    def get_vote_or_unsaved_vote(self, movie, user):
        try:
...
class Vote(models.Model):
    ...
    value = models.SmallIntegerField(choices=VALUE_CHOICES, blank=True)
...
def movie_dir_path(instance, filename):
    return '{}/{}'.format(instance.movie_id, uuid4())


class MovieImage(models.Model):
    image = models.ImageField(upload_to=movie_dir_path)
